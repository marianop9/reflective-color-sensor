...
uint16_t process_samples(uint8_t idx) {
    ...
    char output[ADC_READ_COUNT * 4 + ADC_READ_COUNT - 1 + 4] = {'['};
    int offset = 1;
    uint32_t sum = 0;
    for (int i=0; i<ADC_READ_COUNT; i++) {
        offset += sprintf(output+offset, "%04X", filtered_data[i]);
        // Add comma between elements
        if (i < ADC_READ_COUNT - 1) {
    ...
    printf("Color (%c): %s", colorNames[idx], output);   

    uint16_t avg = sum / (ADC_READ_COUNT - adc_offset);
...
int main() {
    ...
    while (1) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            uint16_t result = process_samples(idx);
            // printf("Result (avg): %02X\n", result);
            printf("Result (avg): %04X\n", result);
