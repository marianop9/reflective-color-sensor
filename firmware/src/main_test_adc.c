...
void handle_adc_finished_irq0() {
    BaseType_t xHigerPrioTaskHasWorken = pdFALSE;
    vTaskNotifyGiveFromISR(led_task_handle, &xHigerPrioTaskHasWorken);

    adc_run(false);
    dma_channel_acknowledge_irq0(adc_dma_chan);
    portYIELD_FROM_ISR(xHigerPrioTaskHasWorken);
}

void init_adc() {
    // Initalize adc and select current channel
    adc_gpio_init(ADC_BASE_PIN + BOARD_ADC_CHAN);

    adc_init();

    adc_select_input(BOARD_ADC_CHAN);
    ...
    const int div = (BOARD_ADC_CLK_HZ / fs) - 1;
    ...
    dma_channel_configure(adc_dma_chan, &dma_cfg,
                          ...
                          ADC_NUM_SAMPLES, // number of transfers
    ...
    irq_set_exclusive_handler(DMA_IRQ_0, handle_adc_finished_irq0);
    irq_set_enabled(DMA_IRQ_0, true);
}

void sample_adc() {
    dma_channel_set_write_addr(adc_dma_chan, adc_buffer, true);
    adc_run(true);
}
...
void led_task(void *arg) {
    while (1) {
        ...
        acp_response_t resp = {
            .type = ACP_RESP_DATA,
            .payload_len_bytes =
                ADC_NUM_SAMPLES * sizeof(adc_buffer[0]) / sizeof(tx_buf[0]),
        };
...
int main() {
    ...
    BaseType_t created =
        xTaskCreate(led_task, "led_task", configMINIMAL_STACK_SIZE, NULL, 10,
                    &led_task_handle);
    if (created != pdPASS) {
        gpio_put(PICO_DEFAULT_LED_PIN, 1);
    }
